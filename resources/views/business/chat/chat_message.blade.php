                                ...
                                <div class="row chat-box tab-pane @if ($loop->first) show active @endif"
                                    ...
                                    <div class="col pe-0 chat-right-aside">
                                        ...
                                        <div class="chat">
                                            ...
                                            <div class="chat-history chat-msg-box custom-scrollbar" id="chat-history-{{ $user->id }}">
                                                <button id="refresh-messages-{{ $user->id }}" class="btn btn-primary mb-2">Refresh Messages</button>
                                                <ul>
                                                    @include('business.chat.message', ['userMessages' => $user_messages])
                                                </ul>
                                            </div>
                                            ...
                                            <script>
                                                $(document).on('click', '#refresh-messages-{{ $user->id }}', function () {
                                                    $.ajax({
                                                        url: '{{ route('business.get.user.message', $user->id) }}', // Laravel route to fetch messages
                                                        type: 'GET',
                                                        success: function (response) {
                                                            // Update the chat history with the new messages
                                                            $('#chat-history-{{ $user->id }} ul').html(response.html);
                                                        },
