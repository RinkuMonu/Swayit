                                    ...
                                    @foreach ($bid_proposals as $proposal)
                                        @if ($proposal->status != 2)
                                            @php
                                                ...
                                                $bidDetails = \App\Models\Bid::where('id', $proposal->bid_id)->first();
                                            @endphp
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <div class="row accept-bid-modal">
                                                                            <div class="col-md-8">
                                                                                ...
                                                                                <button type="button" class="btn btn-success pay-btn" data-bs-toggle="modal"  data-bs-target="#acceptBidPayment{{ $proposal->id }}">Pay Influencer</button>
                                                                            </div>
                                                                            <div class="col-md-4">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($bid_proposals as $list)
                                        @php
                                            ...
                                            $influencer = \App\Models\User::where('id', $list->sender_id)->first();
                                        @endphp
                                        <tr>
                                            <td>{{ $i }}.</td>
                                            @php $i++; @endphp
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <div class="dropdown-menu"
                                                        aria-labelledby="btnGroupVerticalDrop{{ $list->id }}"
                                                        style="">
                                                        <a href="{{ route('business.bid.details', $bid_details->id) }}"
                                                            class="dropdown-item">View Bid</a>
                                                        <button type="button" class="dropdown-item" data-bs-toggle="modal"
                                                            data-bs-target="#viewProposal{{ $list->id }}">View
                                                            Proposal</button>

                                                        @if ($list->status == null)
                                                            <button type="button" class="dropdown-item"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#acceptBid{{ $list->id }}">Accept</button>

                                                            <form id="decline-proposal{{ $list->id }}"
                                                                action="{{ route('business.decline.bid') }}"
                                                                method="POST">
                                                                @csrf
                                                                <input type="hidden" class="form-control" id="id"
                                                                    name="id" value="{{ $list->id }}">
                                                                <input type="hidden" class="form-control" id="receiver_id"
                                                                    name="receiver_id" value="{{ $list->sender_id }}">
                                                                <button type="button" class="dropdown-item"
                                                                    id="decline-proposal-btn{{ $list->id }}">Decline</button>
                                                            </form>
                                                        @elseif($list->status == 1)
                                                            <a href="{{ route('business.create.contract', $list->id) }}"
                                                                class="dropdown-item">Create Contract</a>
                                                        @endif
                                                    </div>
                                                </div>
                                            </td>
                                        ...
                                        <script>
                                            document.getElementById('decline-proposal-btn{{ $list->id }}').addEventListener('click', function(event) {
                                                Swal.fire({
                                                    title: "Are you sure?",
                                                    text: "You won't be able to revert this!",
                                                    icon: "warning",
                                                    ...
                                                    if (result.isConfirmed) {
                                                        // Submit the form
                                                        document.getElementById('decline-proposal{{ $list->id }}').submit();
                                                    }
                                                });
                                            });
                                        </script>
                                        ...
                                        <!-- Modal -->
                                        <div class="modal fade" id="acceptBid{{ $list->id }}" data-bs-backdrop="static"
                                            data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel"
                                            aria-hidden="true">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{ route('business.accept.bid') }}" method="post"
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="mb-2 d-flex">
                                                                <div class="">
                                                                    <input type="text" id="coupon{{ $list->id }}"
                                                                        class="form-control" name="coupon"
                                                                        placeholder="Enter Coupon"
                                                                        style="text-transform: uppercase;">
                                                                </div>
                                                                <div class="mx-2">
                                                                    <button type="button" class="btn btn-primary"
                                                                        id="applyCoupon{{ $list->id }}">Apply
                                                                        Coupon</button>
                                                                </div>
                                                            </div>
                                                            <p id="couponMessage{{ $list->id }}"></p>
                                                            <br>

                                                            <ul class="sub-total">
                                                                <li style="font-size: 22px;">Total <span
                                                                        class="count"><strong>$<span
                                                                                id="totalAmount{{ $list->id }}">{{ $list->amount }}</span></strong></span>
                                                                </li>
                                                            </ul><br>
                                                            ...
                                                            <script>
                                                                $(document).ready(function() {
                                                                    $('#applyCoupon{{ $list->id }}').click(function() {
                                                                        let couponCode = $('#coupon{{ $list->id }}').val();
                                                                        let bidPrice = {{ $list->amount }};
                                                                        ...
                                                                        $.ajax({
                                                                            ...
                                                                            success: function(response) {
                                                                                // alert(response.message);
                                                                                if (response.success) {
                                                                                    $('#totalAmount{{ $list->id }}').text(response
                                                                                        .discounted_price);
                                                                                    $('#amount{{ $list->id }}').val(response.discounted_price);

                                                                                    $('#couponMessage{{ $list->id }}').text(response.message).css(
                                                                                        'color', 'green');
                                                                                } else {
                                                                                    // alert(response.message);
                                                                                    $('#totalAmount{{ $list->id }}').text(response
                                                            ...
                                                            <div class="order-place">
                                                                <button id='pay-btn{{ $list->id }}'
                                                                    class="btn btn-success mt-3" type="button"
                                                                    style="margin-top: 20px; width: 100%;padding: 7px;"
                                                                    onclick="createToken{{ $list->id }}()">PAY
                                                                </button>
                                                            </div>
                                                        </div>
                                                    </form>
                                        ...
                                        <div class="modal fade" id="viewProposal{{ $list->id }}" tabindex="-1"
                                            aria-labelledby="exampleModalLabel" aria-hidden="true">
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="mb-2">
                                                            <p>Budget: &nbsp;<strong> ${{ $list->amount }}.00</strong></p>
                                                        </div>
                                                        <p>{!! $list->message !!}</p>
                                        ...
                                        <script type="text/javascript">
                                            ...
                                            --------------------------------------------
                                            --------------------------------------------*/
                                            function createToken{{ $list->id }}() {
                                                document.getElementById("pay-btn{{ $list->id }}").disabled = true;
                                                stripe.createToken(cardElement).then(function(result) {

                                                    if (typeof result.error != 'undefined') {
                                                        document.getElementById("pay-btn{{ $list->id }}").disabled = false;
                                                        alert(result.error.message);
                                                    }

                                                    /* creating token success */
                                                    if (typeof result.token != 'undefined') {
                                                        document.getElementById("stripe-token-id{{ $list->id }}").value = result.token.id;
                                                        document.getElementById('checkout-form{{ $list->id }}').submit();
                                                    }
                                                });
                                            }
                                        </script>
